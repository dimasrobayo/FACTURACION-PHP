...
<?php
    $query="SELECT * FROM concepto_factura,detalle_facturatmp,marca_concepto WHERE concepto_factura.codigo_marca=marca_concepto.codigo_marca and detalle_facturatmp.n_factura='$codfacturatmp' AND detalle_facturatmp.codigo_concepto=concepto_factura.codigo_concepto";
    $result_lineas=pg_query($query) or die('La consulta fall&oacute;: ' . pg_last_error());
    
    for ($i = 0; $i < pg_num_rows($result_lineas); $i++) {
        $codigo_concepto=pg_result($result_lineas,$i,"codigo_concepto");
        $nombre_concepto=pg_result($result_lineas,$i,"nombre_concepto");
        $marca_concepto=pg_result($result_lineas,$i,"nombre_marca");
        $cantidad=pg_result($result_lineas,$i,"cantidad");
        $monto_concepto=pg_result($result_lineas,$i,"monto_concepto");
        $iva_producto=pg_result($result_lineas,$i,"iva_producto");
        $iva_total= number_format($cantidad*$iva_producto,2,".","");
        $total=number_format(($cantidad*$monto_concepto)+$iva_total,2,".","");
        $total_cantidad=number_format(($cantidad*$monto_concepto),2,".","");
        
        if ($i % 2) { $fondolinea="itemParTabla"; } else { $fondolinea="itemImparTabla"; } ?>
            <tr class="<?php echo $fondolinea?>">
                <td align="center" width="10%"><?php echo $codigo_concepto?></td>
                <td width="32%"><?php echo $nombre_concepto?></td>
                <td align="center" width="5%"><?php echo $cantidad?></td>
                <td align="right" width="10%"><?php echo $monto_concepto?></td>
                <td align="right" width="10%"><?php echo $iva_total?></td>
                <td align="right" width="10%"><?php echo $total_cantidad?></td>
                <?php if ($control!=1) { ?>
                <td align="center" width="4%">
                    <a href="javascript:eliminar_linea(<?php echo $codfacturatmp?>,'<?php echo $codigo_concepto?>',<?php echo $total_cantidad?>)">
                        <img src="../../images/borrar.png" border="0">
                    </a>
                </td>
