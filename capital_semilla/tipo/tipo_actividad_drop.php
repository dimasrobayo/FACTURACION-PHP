...
//seccion para recibir los datos y borrar.
	...
	$datos_borrar= $_GET['codigo_tipo'];

	//se le hace el llamado al archivo de conexion y luego se realiza el enlace.	
	require("conexion/aut_config.inc.php");
	$db_conexion=pg_connect("host=$sql_host dbname=$sql_db user=$sql_usuario password=$sql_pass");	

	$consulta = pg_query("SELECT * FROM tipo_actividad") or die(pg_last_error());
	...
	if ($total_registros == 1){
            ...
            $result_borrar=pg_query("SELECT drop_tipo_actividad($datos_borrar)") or die(pg_last_error());
            pg_close();
	}
    }
?> 
